...
(function($) {
	...
	Calendar.prototype._update = function() {
		...
		//actualização do titulo do calendário
		$('#calendartitle').html(self.getTitle());
		this._update_modal();
	...
	Calendar.prototype._update_modal = function() {
		...
		$('a[data-event-id]', this.context).on('click', function(event) {
				...
				modal
					.on('show.bs.modal', function() {
						...
						//if(_.isFunction(self.options.modal_title)) {
							modal.find("h4").html(event.title);
						//}
					})
			...
			var Iddoc = url.split('/');
			var newHref = iconP.attr('href', HREF+'/'+Iddoc[Iddoc.length-1]);
		});
	};
